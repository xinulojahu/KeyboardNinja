...
int char_to_int(char*, int);  // функция перевода числв char в число int
...
int practice() {
    ...
    min = char_to_int(str, 18);  // запоминаем минимальным количеством
    ...
    for (int i = 1; !feof(practice_done); i++) {
        ...
        if (i % 3 == 0) {  // если строка с количеством попыток
            temp = char_to_int(str, 18);  //то запоминаем эти попытки в temp
